...
CREATE TRIGGER `trg_validate_active_member_sale` BEFORE INSERT ON `SALE` -- before completing sale
 FOR EACH ROW BEGIN
  IF NEW.member_id IS NOT NULL THEN -- if sale is associated with a member (not visitor), check membership status
    IF NOT EXISTS ( -- if membership is active
      SELECT 1 FROM MEMBER 
      WHERE member_id = NEW.member_id 
      AND expiration_date >= CURDATE()
    ) THEN -- say cannot apply discount
