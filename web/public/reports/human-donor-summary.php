...
$page_title = 'Individual Donor Summary';
...
if (isset($_POST['first_name']) && isset($_POST['last_name']) && isset($_POST['email'])) {
    ...
    $stmt = $db->prepare("CALL GetHumanDonorSummary(?, ?, ?)");
    ...
    // Get donor details
    $donor_stmt = $db->prepare("SELECT donor_id, first_name, last_name, email, phone, address FROM DONOR WHERE first_name = ? AND last_name = ? AND email = ? AND is_organization = 0");
    $donor_stmt->bind_param("sss", $first_name, $last_name, $email);
    $donor_stmt->execute();
...
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            ...
            <h1 class="mb-0"><i class="bi bi-person-heart"></i> Individual Donor Summary</h1>
            <p class="text-muted">View donation history for individual donors</p>
        </div>
    ...
    <div class="card mb-4">
        <div class="card-header bg-primary text-white">
            <h5 class="mb-0"><i class="bi bi-search"></i> Search for Donor</h5>
        </div>
        <div class="card-body">
            <form method="POST">
                <div class="row g-3">
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="email" name="email" class="form-control" 
                               ...
                               placeholder="donor@email.com" required>
                    </div>
                    <div class="col-md-12">
                        <button type="submit" class="btn btn-primary">
                            <i class="bi bi-search"></i> Search Donor
                        </button>
                    </div>
                </div>
            </form>
    ...
    <?php if ($search_performed): ?>
        <?php if (empty($donor_info)): ?>
            <div class="alert alert-warning">
                <i class="bi bi-exclamation-triangle"></i> No donor found with the provided information. Please check the details and try again.
            </div>
        <?php else: ?>
            <!-- Donor Information Card -->
            <div class="card mb-4">
                <div class="card-header bg-success text-white">
                    <h5 class="mb-0"><i class="bi bi-person-badge"></i> Donor Information</h5>
                </div>
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-6">
                            <p><strong>Donor ID:</strong> <?= htmlspecialchars($donor_info['donor_id']) ?></p>
                            <p><strong>Name:</strong> <?= htmlspecialchars($donor_info['first_name'] . ' ' . $donor_info['last_name']) ?></p>
            ...
            <?php if (empty($donations)): ?>
                <div class="alert alert-info">
                    <i class="bi bi-info-circle"></i> This donor has not made any donations yet.
                </div>
            <?php else: ?>
                <!-- Donation Summary -->
                ...
                <div class="card mt-4">
                    <div class="card-header bg-warning">
                        <h5 class="mb-0"><i class="bi bi-envelope-heart"></i> Quick Actions</h5>
                    </div>
                    <div class="card-body">
                        <a href="mailto:<?= htmlspecialchars($donor_info['email']) ?>?subject=Thank you for your generous donation" class="btn btn-primary me-2">
